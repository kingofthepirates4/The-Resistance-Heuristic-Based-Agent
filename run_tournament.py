...
def run_single_game(agents):
    """Runs a single game with two agents."""
    # Ensure agents are properly instantiated
    agent_handler = AgentHandler(agents)
    
    # Create a new game instance
    game = Game(agent_handler)

    # Run the game and collect results
    game.run()

    # Check which side won and return the results
    if game.spies_win:
        winner = "Spy"
    else:
        winner = "Resistance"

    # Returning the winner for tracking
